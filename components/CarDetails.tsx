          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel
                  className={
                    "relative max-w-lg max-h-[90vh] w-full overflow-y-auto transform rounded-2xl bg-white text-left shadow-xl p-6 transition-all flew flex-col gap-5"
                  }
                >
                  <button
                    type="button"
                  ...
                  <div className="mt-3 flex flex-wrap gap-4">
                    {Object.entries(car).map(([key, item]) => (
                        <div className="flex justify-between gap-5 w-full text-right" key={key}>
                            <h4 className="capitalize text-grey">{key.split('_').join(" ")}</h4>
                            <p className="text-black-100 font-semibold">{item}</p>
                        </div>
                    ))}
                  </div>
                  </div>
